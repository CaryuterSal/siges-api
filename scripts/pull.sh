#!/bin/bash
set -e

echo "Jalando variables desde SSM..."

get_param() {
  aws ssm get-parameter \
    --name "/siges/$1" \
    --with-decryption \
    --region us-east-1 \
    --query "Parameter.Value" \
    --output text
}

cat > /home/ubuntu/siges/.env <<EOF
SPRING_DATASOURCE_USERNAME=$(get_param SPRING_DATASOURCE_USERNAME)
SPRING_DATASOURCE_PASSWORD=$(get_param SPRING_DATASOURCE_PASSWORD)
SPRING_DATASOURCE_URL=$(get_param SPRING_DATASOURCE_URL)
SPRING_DATA_REDIS_URL=$(get_param SPRING_DATA_REDIS_URL)
DOCKER_HUB_USERNAME=$(get_param docker/username)
SECURITY_JWT_SECRET=$(get_param SECURITY_JWT_SECRET)
SPRING_MAIL_USERNAME=$(get_param SPRING_MAIL_USERNAME)
SPRING_MAIL_PASSWORD=$(get_param SPRING_MAIL_PASSWORD)
GOOGLE_APPLICATION_CREDENTIALS=/app/secrets/firebase-service-account.json
AWS_REGION=$(get_param AWS_REGION)
AWS_S3_BUCKET=$(get_param AWS_S3_BUCKET)
AWS_CLOUDFRONT_DOMAIN=$(get_param AWS_CLOUDFRONT_DOMAIN)
PASSWORD_RECOVERY_WEB_SUCCESS_URL=$(get_param PASSWORD_RECOVERY_WEB_SUCCESS_URL)
PASSWORD_RECOVERY_WEB_USED_URL=$(get_param PASSWORD_RECOVERY_WEB_USED_URL)
PASSWORD_RECOVERY_WEB_EXPIRED_URL=$(get_param PASSWORD_RECOVERY_WEB_EXPIRED_URL)
PASSWORD_RECOVERY_MOBILE_SUCCESS_URL=$(get_param PASSWORD_RECOVERY_MOBILE_SUCCESS_URL)
PASSWORD_RECOVERY_MOBILE_USED_URL=$(get_param PASSWORD_RECOVERY_MOBILE_USED_URL)
PASSWORD_RECOVERY_MOBILE_EXPIRED_URL=$(get_param PASSWORD_RECOVERY_MOBILE_EXPIRED_URL)
EOF

echo "Escribiendo credenciales de Firebase..."
mkdir -p /home/ubuntu/siges/secrets
get_param firebase/credentials > /home/ubuntu/siges/secrets/firebase-service-account.json
chmod 644 /home/ubuntu/siges/secrets/firebase-service-account.json

echo "Haciendo docker login..."
DOCKER_TOKEN=$(get_param docker/token)
DOCKER_USER=$(get_param docker/username)
echo "$DOCKER_TOKEN" | docker login -u "$DOCKER_USER" --password-stdin

echo "Limpiando imágenes antiguas..."
docker image prune -a -f

echo "Jalando imagen..."
cd /home/ubuntu/siges
docker compose -f compose.yaml -f compose.prod.yaml pull