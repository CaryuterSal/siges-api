...
public class UserManagementController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "422",
                    description = "Old password does not matches with current user password",
                    content = @Content(schema = @Schema(implementation = ProblemDetail.class)))
    })
    @Operation(summary = "Updates self password")
    @PatchMapping("/users/me/password")
    public AuthenticatedResponse updatePassword(@RequestBody @Valid PasswordUpdateRequest request, HttpServletRequest servletRequest) {
        return managementService.updatePassword(request, servletRequest.getRemoteAddr());
    }
}
