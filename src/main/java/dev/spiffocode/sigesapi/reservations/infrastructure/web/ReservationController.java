...
public class ReservationController {
    ...
    public Page<@NonNull ReservationResponse> getReservations(
            ...
            @Schema(description = "Filter by petitioner user ID")
            Long petitionerId,

            @RequestParam(required = false)
            @Schema(description = "Filter by petitioner name (partial, case-insensitive)")
            String petitionerName,

            @RequestParam(required = false)
            @Schema(description = "Filter by exact reservation date")
            LocalDate date,

            @RequestParam(required = false)
            @Schema(description = "Filter reservations from this date (inclusive). Ignored if 'date' is set.")
            LocalDate dateFrom,

            @RequestParam(required = false)
            @Schema(description = "Filter reservations until this date (inclusive). Ignored if 'date' is set.")
            LocalDate dateTo,

            @RequestParam(required = false)
            @Schema(description = "Filter by reservation status")
            Status status,

            @RequestParam(required = false)
            @Schema(description = "Filter by reservable ID")
            Long reservableId,

            @RequestParam(required = false)
            @Schema(description = "Filter by grouping type (SINGLE or GROUP)")
            GroupingType type,
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Reservation created", useReturnTypeSchema = true,
                    headers = @Header(name = "Location", description = "URI to retrieve the created reservation")),
    ...
    public ReservationResponse approveReservation(@PathVariable Long id) {
    ...
    @Operation(summary = "Cancel a reservation",
            description = "Petitioner can cancel their own reservation. Admins can cancel any.")
    @ApiResponses({
    ...
    @Operation(summary = "Edit a note on a reservation",
            description = "Users can only edit their own notes. Admins can edit any note.")
    @ApiResponses({
